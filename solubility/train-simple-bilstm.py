...
from solubility.data import load_ESOL
from solubility.model import build_simple_bilstm_model
...
def train_simple_bilstm(pad_to, lstm_hidden, lr, loss, savefigto):
    ...
    plt.savefig(Path(savefigto)/f'./solubility_simple_bilstm-{pad_to}-{lstm_hidden}-{lr}-{loss}-{MSE:.3f}.png')
    plt.close()
...
if __name__ == "__main__":
    ...
    with open('solubility-simple-bilstm-summary.csv', 'w') as fout:
        ...
        lines = []
        for fname in Path(savefigto).glob('solubility_simple_bilstm*.png'):
